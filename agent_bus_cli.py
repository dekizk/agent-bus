"""Approachable, read-only operations CLI for agent-bus."""
...
def _add_common_options(parser: argparse.ArgumentParser, *, defaults: bool = False) -> None:
    ...
    parser.add_argument(
        "--url",
        default=(os.environ.get("AGENT_BUS_URL", DEFAULT_URL) if defaults else default),
        help="bus URL (default: AGENT_BUS_URL or http://127.0.0.1:8765)",
    )
    parser.add_argument(
        "--token",
